				...
				if (token == "VALIDEFI_NEWS_XS345&DS^89212nd") {
					...
					axios
						...
						.then((response) => {
							return res
								.status(200)
								.json({ status: true, data: response.data.results });
						})
						.catch((error) => {
							console.log(error);
							return res
					...
					return res
						.status(400)
						.json({ status: false, message: "Malformed Token, Try again" });
				}
			} else {
				return res
					.status(400)
					.json({ status: false, message: "Need Token to access Api" });
			}
		});
					...
					if (!(network && address)) {
						...
						axios
							...
							.then((response) => {
								return res
									.status(200)
									.json({ status: true, data: response.data.transactions });
							})
							.catch((error) => {
								console.log(error);
								return res
									.status(500)
									.json({ status: false, message: "Service Unavialable" });
							});
					}
				} else {
					return res
